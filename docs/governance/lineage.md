...
- **Observability is out-of-band and read-only (v1.3):** the lag exporter (`src/pipeline/lag_exporter.py`) observes consumer groups via non-subscribing consumers (`committed()` never joins a group) and the bank DB via `SELECT COUNT`/`MAX(scored_at)` — it writes nothing and cannot affect delivery semantics or rebalancing. Prometheus/Grafana (compose profile `obs`) sit entirely outside the data path. See ADR 0004.
