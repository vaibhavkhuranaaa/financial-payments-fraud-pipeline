...
- The model itself is not directly affected (it never sees `card_token` as a feature — only the *aggregates* keyed by it), but a rotation effectively **cold-starts every card's windowed features** the moment it takes effect, which briefly degrades feature quality (all `_1m`/`_10m`/`_1h` aggregates read as zero/empty until enough post-rotation history accumulates).
