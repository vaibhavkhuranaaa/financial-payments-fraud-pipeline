...
## Next step (exact resume sequence) — post-v1.2
v1.2 is tagged. Next in impact order (per `docs/HANDOFF.md` roadmap; delivery semantics = item 1 is DONE, folded into v1.2):
**observability stack** (Prometheus + Grafana scraping the existing `/metrics`, consumer-lag export — the flagship DE signal, structured logging) or **schema registry + typed events** — write a ticket before building either; neither has one yet.

1. Read `docs/HANDOFF.md` roadmap; pick the top unticketed item, write `docs/tickets/13-*.md`, then build.
