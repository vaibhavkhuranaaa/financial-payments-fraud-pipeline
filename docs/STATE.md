...
## In flight
- **v1.5 COMPLETE (2026-07-19). Nothing in flight.** Session used 3 Sonnet subagents (tickets 14/15/17) in isolated git worktrees, orchestrator-reviewed and squash-merged; their `worktree-agent-*` branches and `.claude/worktrees/` dirs still exist locally (deleting branches = user-approval item; safe to `git worktree remove` + `git branch -D` if you don't need the audit trail).
...
## Next step (exact resume sequence) — post-v1.5
v1.3, v1.4, v1.5 all tagged and pushed (session 2026-07-19). **The only remaining roadmap item is ticket 18 (schema registry + typed events)** — deliberately NOT started: its own spec forbids beginning without budget to finish and E2E-verify both demo modes (a half-migrated wire format is worse than JSON). Everything else in the hardening roadmap is shipped.

1. Read `docs/HANDOFF.md`, then `docs/tickets/18-schema-registry.md`; write ADR 0006 (design decisions are pre-framed in the ticket) before any code.
